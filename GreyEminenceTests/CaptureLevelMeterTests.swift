...
final class CaptureLevelMeterTests: XCTestCase {
    ...
    func testResetClearsEverything() {
        ...
        meter.reset()
        XCTAssertEqual(meter.snapshot, CaptureLevelMeter.Snapshot())
    }
    ...
    func testBacklogSecondsFromTheSystemTapsBufferShape() {
        ...
        let seconds = CaptureLevelMeter.backlogSeconds(buffers: nineMinutes, framesPerBuffer: 512, sampleRate: 48000)
        XCTAssertEqual(seconds, 540, accuracy: 0.5)
        XCTAssertGreaterThan(seconds, CaptureLevelMeter.backlogWarningSeconds)
    }

    func testBacklogNeverGoesNegativeAndAHealthyLoopIsQuiet() {
        XCTAssertEqual(CaptureLevelMeter.backlog(delivered: 100, consumed: 103), 0)
        let healthy = CaptureLevelMeter.backlogSeconds(buffers: CaptureLevelMeter.backlog(delivered: 1000, consumed: 998), framesPerBuffer: 512, sampleRate: 48000)
        XCTAssertLessThan(healthy, CaptureLevelMeter.backlogWarningSeconds)
        XCTAssertEqual(CaptureLevelMeter.backlogSeconds(buffers: 5, framesPerBuffer: 512, sampleRate: 0), 0)
    }
